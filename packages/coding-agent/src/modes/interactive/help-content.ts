...
export function buildHelpMarkdown(input: { extensionCommands: SlashCommandInfo[] }): string {
	...
	return [
		...
		"Labels display as `displayName (id)`; pins, removal, refresh and session affinity always use the unchanged ID. Labels are trimmed, limited to 80 characters, cannot contain control or formatting characters, and must be unique within the provider (case-insensitive). Environment accounts cannot be renamed.",
		"`/gpt-account add` and `/claude-account add` offer an optional display name after saving the account, including Claude imports. Leave it blank or cancel to keep the successful login without a label. Legacy accounts display only their ID.",
	].join("\n\n");
}
