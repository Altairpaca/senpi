...
export interface GrepEngineMatch { path: string; line: number; column?: number; text: string; isContext: boolean; truncated: boolean }  // column = 1-based byte column of the first submatch on the FIRST physical line of a match only; later lines of a multiline match and context rows carry column undefined
export interface GrepEngineFileCount { path: string; count: number | null; limitReached: boolean }
export interface GrepEngineWarning { path?: string; code: string; message: string }
export interface GrepEngineResult {
  ...
  limitReached: boolean; perFileLimitReached: boolean;
  skippedOversized: number; prefixSearched: number; skippedBinary: number;
  ...
  effectivePattern: string; patternKind: "regex" | "sanitized" | "literal"; regexEngine: "rust" | "pcre2";
}
export type GrepEngineErrorCode = "UNSUPPORTED_REGEX" | "INVALID_PATTERN" | "INVALID_GLOB" | "UNKNOWN_TYPE" | "PATH_NOT_FOUND" | "ABORTED" | "ENGINE_UNAVAILABLE";
// @ts-ignore erasableSyntaxOnly rejects the frozen parameter property
export class GrepEngineError extends Error { constructor(readonly code: GrepEngineErrorCode, message: string) { super(message); } }
export interface GrepEngine { readonly name: "native" | "rg"; search(request: GrepEngineRequest, signal?: AbortSignal): Promise<GrepEngineResult> }
