...
describe("goal monitor scheduling notice over RPC", () => {
	...
	it("emits a scheduling event and one durable RPC entry", async () => {
		...
		expect(scheduleEvents).toEqual([expect.objectContaining({ delayMs: 3_570_000, dueAtMs: 3_570_000 })]);
		...
		expect(records).toContainEqual(
			expect.objectContaining({
				...
				entry: expect.objectContaining({
					...
					data: expect.objectContaining({ phase: "scheduled", dueAtMs: 3_570_000, iteration: 1 }),
				}),
			}),
		);
