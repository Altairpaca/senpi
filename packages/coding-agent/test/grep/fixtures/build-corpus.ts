...
export async function buildCorpus(options: { git?: boolean } = {}): Promise<{ root: string; cleanup(): Promise<void> }> {
  ...
  await put("late-nul.bin", Buffer.concat([Buffer.from("needle\n"), Buffer.alloc(70000, "a"), Buffer.from("\n\0\nneedle\n")]));
  const big = Buffer.alloc(5 * MiB, 97); Buffer.from("needle").copy(big, 100); Buffer.from("needle").copy(big, 4 * MiB + 100); await put("big.txt", big);
  const late = Buffer.alloc(5 * MiB, 97); Buffer.from("needle").copy(late, 100); late[4500000] = 0; await put("big-late-nul.txt", late);
