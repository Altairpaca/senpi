...
**gpt-image-gen** is contributed by the `imagegen` builtin extension when image-generation credentials exist (a stored OpenAI key, `OPENAI_API_KEY`, or a configured OpenAI-compatible gateway). It is the prompt-crafting guide for GPT Image 2.5: tool routing (native `image_generation` server tool vs. the client `generate_image` tool), Flare-vs-Sunburst selection, prompt structure with a specificity policy (normalize specific requests, add concreteness only to generic ones), verbatim quoted render text, reference roles and end-state edits, transparent assets, output formats, multi-turn refinement, and a result checklist. For `generate_image`, `model` selects `gpt-image-2.5-flare` (default, fast), `gpt-image-2.5-sunburst` (precise edits and final assets), or legacy `gpt-image-2`; `quality` accepts `auto`, `low`, `medium`, `high`, `xhigh`, or `max` (the last two require 2.5); `size` accepts `auto`, presets, or validated custom dimensions; `background` (`auto`, `transparent`, `opaque`), `output_format` (`png`, `jpeg`, `webp`), `output_compression` (0-100, jpeg/webp only), and `moderation` (`auto`, `low`) shape the output, and the saved file's extension follows the delivered container. `reference_image_paths` supplies 1-5 local PNG/JPEG/WEBP files (up to 50 MB each) for edits and references, and `mask_image_path` adds an alpha-channel mask for local repaints. Only the native server tool returns a `revised_prompt`; the client tool reports one only when the provider sends it.
