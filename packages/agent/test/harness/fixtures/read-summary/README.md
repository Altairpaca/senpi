...
- `binary-sizes.ts` retains the historical prototype probe; the integrated bake-off no longer invokes it. `omp-item1 --case compiled-parity` builds the actual release entry graph with identical baseline/candidate flags for every shipped target. The required package/theme layout and version smoke are recorded explicitly.
