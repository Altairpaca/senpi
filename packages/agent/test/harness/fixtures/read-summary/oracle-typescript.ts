...
export function typescriptOracle(source: string, language: string) {
	const kind = language === "tsx" ? ts.ScriptKind.TSX : language === "js" ? ts.ScriptKind.JS
		: language === "json" ? ts.ScriptKind.JSON : ts.ScriptKind.TS;
	...
	const protect = (node: ts.Node, end = node.end) => protectedRanges.push({
		start: line(node.getStart(file)), end: line(Math.max(node.getStart(file), end - 1)),
	});
	const visit = (node: ts.Node) => {
		...
		if (ts.isTypeNode(node) || bindingPattern(node) || ts.isParameter(node) || ts.isHeritageClause(node)
			|| ts.isDecorator(node) || ts.isImportDeclaration(node) || ts.isExportDeclaration(node)) {
			protect(node);
			return;
		}
		if (ts.isFunctionLike(node)) {
			const body = "body" in node ? node.body : undefined;
			if (!body) { protect(node); return; }
		...
		if (ts.isBlock(node) || ts.isModuleBlock(node) || ts.isCaseBlock(node)
			...
			const open = children.find((c) => c.kind === ts.SyntaxKind.OpenBraceToken || c.kind === ts.SyntaxKind.OpenBracketToken);
			const close = children.findLast((c) => c.kind === ts.SyntaxKind.CloseBraceToken || c.kind === ts.SyntaxKind.CloseBracketToken);
			if (open && close) candidates.push({ start: line(open.getStart(file)) + 1, end: line(close.getStart(file)) - 1, kind: "body" });
		}
	...
	return {
		...
		allowed: candidates.filter((range) => range.end >= range.start && !protectedRanges.some((header) => overlaps(range, header))),
	};
}
